...
router.post('/capturar', async (req, res) => {
  try {
    await getDatabase();
    const novasOfertas = await dealCapture.captureAll();
    res.json({ message: `Captura concluída. ${novasOfertas} novas ofertas encontradas.`, novasOfertas });
  } catch (err) {
    res.status(500).json({ error: 'Erro na captura: ' + err.message });
  }
});
